...
function prepare_seeder_files {
    ...
    local REALSTORE=$TMP_DIR/peers/${1}_real
    mkdir -p $STORE $REALSTORE

    $LFS -o realstore=$REALSTORE,big_writes $STORE
    truncate -s $(($FILE_SIZE*1024*1024)) $STORE/bbbbbbbb_111gb_1024
    $SWIFT -f $STORE/bbbbbbbb_111gb_1024 -m -z 1024 --debug
    
    HASHES[$1]=`cat $STORE/bbbbbbbb_111gb_1024.mbinmap | head -2 | tail -1 | cut -d " " -f 3`

    mv $STORE/bbbbbbbb_111gb_1024 $STORE/${HASHES[$1]}
    mv $STORE/bbbbbbbb_111gb_1024.mhash $STORE/${HASHES[$1]}.mhash
    mv $STORE/bbbbbbbb_111gb_1024.mbinmap $STORE/${HASHES[$1]}.mbinmap
...
fusermount -u $TMP_DIR/peers/seeder
